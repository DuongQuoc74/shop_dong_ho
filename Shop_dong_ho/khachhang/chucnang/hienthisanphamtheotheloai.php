...
if ($_GET['id']) {
	...
	$sql = "SELECT * FROM sanpham WHERE danhmuc ='$matl' LIMIT $begin,4";
	$qr = mysqli_query($conn,$sql);
    while ($row = mysqli_fetch_array($qr)){
 ?>
<div class="hienthisanpham2">
	 ...
	 ?>
	  ...
	  <div class="divphantrang">
	 	<ul class="phantrang">
	 		...
	 		for ($i=1; $i <= $trang ; $i++) { 
	 		?>
	 		<li <?php if($i==$page){echo 'style="background:red"';}else{ echo '';} ?> ><a href="http://localhost:81/Shop_dong_ho/khachhang/index.php?menupage=danhmucsanpham&trangchu&id=<?php echo $matl ?>&trang=<?php echo $i ?>"><?php echo $i ?></a></li>
	 		<?php 
	 			}
	 		 ?>
